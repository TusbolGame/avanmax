...
if (isset($_POST['save'])) {
	...
	if (isset($_POST['developer_tools-enabled'])) {
        $enabled = 1;
    } else {
        $enabled = 0;
    }
    if (isset($_POST['developer_tools-alert'])) {
        $alert = 1;
    } else {
        $alert = 0;
    }
    $message = $_POST['developer_tools-message'];
    $update  = $mysqli->query("UPDATE `$table` SET enabled='$enabled', alert='$alert', message='$message' WHERE id=13");
...
?> />
												...
												<div class="col-md-10">
												    ...
												    Enable this option if your website does not have a jquery file included
												</div>
												</div>
											</div>

								<button type="button submit" name="save" class="mb-xs mt-xs mr-xs btn btn-flat btn-success btn-md btn-block"><i class="fas fa-floppy"></i>&nbsp;&nbsp;Save</button><br />
...
<?php
    if ($row['function'] == "rightclick") {
        ...
        echo '<i class="fas fa-object-group"></i> Website displays in iFrames';
    } elseif ($row['function'] == "selecting") {
        echo '<i class="fas fa-arrows-alt-h"></i> Selecting';
    } elseif ($row['function'] == "developer_tools") {
        echo '<i class="fas fa-window-maximize"></i> Developer Tools (Console)';
    }
?>
                                       </h3>
                                    </div>
...
<?php
    if ($row['function'] == "rightclick") {
        ...
        echo 'Prevent using Browser\'s Developer Tools (Console) on your website';
    }
?>
										</p>
									    <hr>
